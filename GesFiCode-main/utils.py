...
def trainer(
    ...
    if args is not None and args.dataset == "widar":
        ...
        train_list = (
            args.data_path
            if (args and args.data_path)
            else r"C:\Users\G\Downloads\GesFiCode-main-v2\GesFiCode-main\Processed_Data"
        )
        dataset_source = datatrcsie(data_list=train_list, transform=img_transform)
        test_list = train_list
        dataset_target = datatecsie(data_list=test_list, transform=img_transformte)
        # XRF55 源域评估集
        dataset_source_eval = datatrcsie(
            data_list=train_list, transform=img_transformte
        )
    ...
    if data_frac < 1.0:
        ...
        class SubsetProxy(torch.utils.data.Dataset):
            ...
            def set_labels_by_index(
                self, tlabels=None, tindex=None, label_type="domain_label"
            ):
        ...
        print(
            f"[Data Fraction] Using {data_frac * 100:.0f}% of training data: {n_keep}/{n_total}"
        )

    train_loader = torch.utils.data.DataLoader(
        dataset=dataset_source, batch_size=batch_size, shuffle=True, num_workers=2
    )
    test_loader = torch.utils.data.DataLoader(
        dataset=dataset_target, batch_size=batch_size, shuffle=False, num_workers=2
    )
    source_eval_loader = torch.utils.data.DataLoader(
        dataset=dataset_source_eval, batch_size=batch_size, shuffle=False, num_workers=2
    )
    ...
    phys_mask_aug = Physical_Mask_Augment(
        variance_percentile=var_pct, mask_ratio=mask_r, patch_size=p_size
    )
    ...
    with open(acc_file, "w") as f:
        for round in range(total_epoch):
            ...
            if ablation == "M0":
                ...
                loss_list = ["total", "cls"]
                print_row(
                    ["epoch"] + [item + "_loss" for item in loss_list], colwidth=15
                )
                for step in range(4):
                    ...
                    print_row(
                        [step] + [loss_result_dict.get(item, 0) for item in loss_list],
                        colwidth=15,
                    )
                ...
                if round <= 2 and use_supcon:
                    ...
                    for step in range(2):
                        ...
                        print_row(
                            [step]
                            + [loss_result_dict.get(item, 0) for item in loss_list],
                            colwidth=15,
                        )
                else:
                    loss_list = ["total", "cls"]
                    ...
                    for step in range(1):
                        for inputs, labels, pdlables, item in train_loader:
                            loss_result_dict = trainmodel.update_d(
                                inputs, labels, pdlables, opta, skip_grl=skip_grl_s2
                            )
                        print_row(
                            [step]
                            + [loss_result_dict.get(item, 0) for item in loss_list],
                ...
                for step in range(1):
                    ...
                    print_row(
                        [step] + [step_vals.get(item, 0) for item in loss_list],
                        colwidth=15,
                    )
            ...
            f.write(
                f"Round {round}: src_acc={src_acc:.6f}, tgt_acc={tgt_acc:.6f}, best_tgt={bestac:.6f}\n"
            )
            f.flush()
    ...
    gesture_names = (
        GESTURE_NAMES_WIDAR
        if (args and args.dataset == "widar")
        else GESTURE_NAMES_XRF55
    )
